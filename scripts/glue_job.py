...
if dataframe.head(1):
    ...
    transformed_df = dataframe \
        .withColumn("order_id",
                        when(
                                col("order_id.long").isNotNull(),          
                                col("order_id.long").cast("string")       
                            ).otherwise(
                                regexp_replace(col("order_id.string"), "#", "") 
                            )) \
    ...
    transformed_dyframe = DynamicFrame.fromDF(transformed_df, glueContext, "transformed_dyframe")
